...
// data 영역에서 header/variable-offset table을 지나서 fixed data 시작 지점 반환
...
struct heap_classrepr_cache
{
  ...
  int num_hash;
  HEAP_CLASSREPR_HASH *hash_table; // class_oid로 엔트리 탐색(HASH 하나에 한 class_oid가 있는 것 같지 않음. 여러 class_oid 엔트리를 담은 버킷이 있는듯)
...
static void heap_bestspace_add_candidate (cubstorage::bestspace_entry * candidates, std::size_t * num_candidates,
					  std::size_t max_candidates, cubstorage::bestspace_entry * candidate);
static int heap_bestspace_fix_page (THREAD_ENTRY * thread_p, const HFID * hfid, const VPID * vpid,
...
static int heap_load_bestspace (THREAD_ENTRY * thread_p, HFID * hfid, const HEAP_HDR_STATS * header,
				cubstorage::bestspace_entry * entries, cubstorage::bestspace_entry * candidates);

// *INDENT-OFF*
static int heap_update_bestspace (THREAD_ENTRY * thread_p, const HFID * hfid, cubstorage::bestspace *bestspace);
static int heap_update_bestspace_registry_entry (const HFID * hfid, cubstorage::bestspace *bestspace, void *args);
...
heap_scan_pb_latch_and_fetch_debug (THREAD_ENTRY * thread_p, const VPID * vpid_ptr, PAGE_FETCH_MODE fetch_mode,
				    ...
				    const char *caller_func)
#endif				/* !NDEBUG */
{ // 파라미터로 전달된 vpid_ptr를 lock 모드에 따라 fix해줌
  PAGE_PTR pgptr = NULL;
...
heap_classrepr_lock_class (THREAD_ENTRY * thread_p, HEAP_CLASSREPR_HASH * hash_anchor, const OID * class_oid)
{
 ...
 // 결국 위에서 class_oid에 대한 lock_entry를 획득하지 못했다면 새로 등록해줌
  ...
  cur_lock_entry->lock_next = hash_anchor->lock_next; // 내 엔트리 lock_next로 hash_anchor의 다음 lock next 할당
  hash_anchor->lock_next = cur_lock_entry; // hash_anchor의 바로 다음 순서로 내 lock_entry 설정(이렇게 되면 내가 바로 1번이 되고 그 이후에 기존 lock_next가 줄줄이 붙게 됨)
...
{ // class_recdes가 파라미터로 전달됐다면 repr만 찾아서 반환하고, class_recdes가 NULL로 전달됐다면 root_class_oid를 기준으로 원하는 class_oid의 repr 반환
  RECDES peek_recdes;
  RECDES *recdes = NULL;
  HEAP_SCANCACHE scan_cache;
  ...
  if (last_reprid != NULL)
    {
      *last_reprid = NULL_REPRID; // last_reprid 초기값은 무조건 NULL_REPRID
    }
  ...
  if (class_recdes != NULL)
    ...
    {
      heap_scancache_quick_start_root_hfid (thread_p, &scan_cache); // root_hfid로 scacache 생성(S_LOCK)
      if (heap_get_class_record (thread_p, class_oid, &peek_recdes, &scan_cache, PEEK) != S_SUCCESS) // class_oid의 record 획득.
	{                                                                                         // 작동 방식은 다른 oid record 획득하는거랑 크게 다르지 않은듯
	  goto end;
	}
      recdes = &peek_recdes;
    }

  repr = or_get_classrep (recdes, reprid); // 위에서 획득한 record로 classrep 획득. reprid는 획득 안 함
  if (last_reprid != NULL)
    {
      *last_reprid = or_rep_id (recdes); // [질문] 이 레코드 자체가 구버전이면 그건 테이블 전체의 last_reprid가 아닐 수 있을 것 같은데 왜 last_reprid지?
    }                                   // [추측] 이 함수는 class_oid의 record를 얻어오기 때문에 해당 record의 repid는 무조건 last_reprid일듯

end:
  if (class_recdes == NULL)
    {
      heap_scancache_end (thread_p, &scan_cache); // scan_cache 값들 정리 + 초기화
    }
  return repr;
}
...
search_begin:
  ...
  for (cache_entry = hash_anchor->hash_next; cache_entry != NULL; cache_entry = cache_entry->hash_next) // [의문] hash_next는 해당 class_oid hash에 대해 대기하고 있는 다른 트랜잭션일까?
  ...
  if (cache_entry == NULL) // 위 전역 엔트리에서 원하는 class_oid를 찾지 못했다면
      ...
      if (repr_from_record == NULL)
	{
	  ...
	  VPID_GET_FROM_OID (&vpid_of_class, class_oid); // class_oid로 class_oid가 저장된 페이지의 vpid 획득
	  page_of_class = pgbuf_fix (thread_p, &vpid_of_class, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_CONDITIONAL_LATCH);
	  if (page_of_class == NULL) // [질문] 위에서 vpid를 얻었는데 LATCH에 실패할 수도 있나? 다른 스레드에서 write latch를 잡은 경우?(주석 참고)
	    {
	  ...
	  repr_from_record = heap_classrepr_get_from_record (thread_p, &last_reprid, class_oid, class_recdes, reprid); // class_oid로 last_repr/last_reprid 획득. reprid는 획득 안 함
	  if (repr_from_record == NULL)
	    {
	      ASSERT_ERROR ();
	  ...
	  if (reprid == NULL_REPRID) // reprid가 NULL이면
	    {
	      reprid = last_reprid; // last_reprid로 반환(NULL을 줬다는건 last를 원한다는 뜻 같음)
	    }
...
/*
 * heap_page_is_bestspace () - Check whether a heap page is a bestspace storage page.
 *
 * return	  : true if page has the bestspace page flag
 * page_heap (in) : Heap page
 */
bool
heap_page_is_bestspace (THREAD_ENTRY * thread_p, PAGE_PTR page_heap)
{
  RECDES recdes;
...
heap_vpid_remove (THREAD_ENTRY * thread_p, const HFID * hfid, HEAP_HDR_STATS * heap_hdr, VPID * rm_vpid)
{
  ...
  if (VPID_EQ (&heap_hdr->last_vpid, rm_vpid))
    {
      /* If the page is the last page of the heap file, update the hint */
      heap_hdr->last_vpid = rm_chain->prev_vpid;
    }
...
heap_remove_page_on_vacuum (THREAD_ENTRY * thread_p, PAGE_PTR * page_ptr, HFID * hfid)
{
  ...
  if (VPID_EQ (&heap_hdr.last_vpid, &page_vpid))
    {
      VPID_COPY (&heap_hdr.last_vpid, &prev_vpid);
    }
...
/*
 * heap_bestspace_clear_candidates () - Clear persistent bestspace candidates.
 *   return:
 *   heap_hdr(in/out): Heap header
 */
static void
heap_bestspace_clear_candidates (cubstorage::bestspace_entry * candidates, std::size_t * num_candidates,
				 std::size_t max_candidates)
{
  std::size_t i;
  ...
  for (i = 0; i < max_candidates; i++)
    {
      candidates[i].set_null ();
    }
}

/*
 * heap_bestspace_add_candidate () - Add one persistent bestspace candidate in descending freespace order.
 *   return:
 *   heap_hdr(in/out): Heap header
 *   candidate(in): Candidate to add
 */
static void
heap_bestspace_add_candidate (cubstorage::bestspace_entry * candidates, std::size_t * num_candidates,
			      std::size_t max_candidates, cubstorage::bestspace_entry * candidate)
{
  std::size_t pos;
  ...
  pos = MIN (*num_candidates, max_candidates - 1);
  while (pos > 0 && candidates[pos - 1].freespace < candidate->freespace)
    {
...
/*
 * heap_bestspace_fix_page () - Ordered-fix a persistent bestspace metadata page.
...
/*
 * heap_create_bestspace () - Create a bestspace for heap file
 *   return:
 *   hfid(in/out):
 *   class_oid(in):
 */
static int
heap_create_bestspace (THREAD_ENTRY * thread_p, HFID * hfid, HEAP_HDR_STATS * heap_hdr,
		       cubstorage::bestspace_entry * first_entry)
{
  ...
  HEAP_CHAIN page_chain;
  VPID header_vpid;
  VPID null_vpid;
  const VPID *prev_vpid;
  ...
  /* candidates */
  heap_bestspace_clear_candidates (heap_hdr->bestspace.candidates, &heap_hdr->bestspace.num_candidates,
				   cubstorage::bestspace::MAX_CANDIDATES_QUEUE_SIZE);
  ...
  if (heap_hdr->bestspace.num_pages > max_pages)
    {
      assert (false);
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_HEAP_UNABLE_TO_CREATE_HEAP, 1,
	      fileio_get_volume_label (hfid->vfid.volid, PEEK));
  ...
  for (i = 0; i < (int) max_pages; i++)
    {
      VPID_SET_NULL (&heap_hdr->bestspace.pages[i]);
    }
  page_chain.class_oid = heap_hdr->class_oid;
  VPID_SET_NULL (&page_chain.prev_vpid);
  VPID_SET_NULL (&page_chain.next_vpid);
  ...
  error =
    file_alloc_multiple (thread_p, &hfid->vfid, heap_vpid_init_new, &page_chain, heap_hdr->bestspace.num_pages,
			 heap_hdr->bestspace.pages);
  if (error != NO_ERROR)
    {
      ASSERT_ERROR ();
      return error;
  ...
  for (i = 0; i < (int) heap_hdr->bestspace.num_pages; i++)
    {
      ...
      next_vpid = (i == (int) heap_hdr->bestspace.num_pages - 1) ? &null_vpid : &heap_hdr->bestspace.pages[i + 1];

      error =
	heap_add_chain_links (thread_p, hfid, &heap_hdr->bestspace.pages[i], next_vpid, prev_vpid,
			      &chain_page_watcher, false, false);
      if (error != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  return error;
  ...
  assert (heap_hdr->bestspace.num_pages > 0);
  heap_hdr->next_vpid = heap_hdr->bestspace.pages[0];
  heap_hdr->last_vpid = heap_hdr->bestspace.pages[heap_hdr->bestspace.num_pages - 1];

  /* insert entries */
  entries = (cubstorage::bestspace_entry *) PTR_ALIGN (pgbuf, MAX_ALIGNMENT);
  remaining_entries = max_entries;
  for (i = 0; i < (int) heap_hdr->bestspace.num_pages; i++)
    {
      entries_count = MIN (remaining_entries, entries_per_page);
      ...
      for (j = 0; j < entries_count; j++)
	{
      ...
      /* first entry is the heap header page when the bestspace being created */
      if (i == 0)
	{
	  entries[0] = *first_entry;
	}

      error = heap_bestspace_fix_page (thread_p, hfid, &heap_hdr->bestspace.pages[i], &page_watcher);
      if (error != NO_ERROR)
	{
	  return error;
	}
      page_ptr = page_watcher.pgptr;

      recdes.area_size = entries_count * sizeof (cubstorage::bestspace_entry);
      recdes.length = recdes.area_size;
      recdes.type = REC_HOME;
      recdes.data = (char *) entries;
      ...
      if (sp_success != SP_SUCCESS || slotid != HEAP_BESTSPACE_ENTRIES_SLOTID)
	{
      ...
      remaining_entries -= entries_count;
    }
...
/*
 * heap_update_bestspace_chain () - Update chain records of persistent bestspace pages.
 *   return: NO_ERROR if successful, error code otherwise
 */
static int
heap_update_bestspace_chain (THREAD_ENTRY * thread_p, const OID * class_oid, const HFID * hfid, const VPID * pages,
			     int num_pages)
{
  PGBUF_WATCHER page_watcher;
  PAGE_PTR page_ptr = NULL;
  ...
  for (i = 0; i < num_pages; i++)
      ...
      if (VPID_ISNULL (&pages[i]))
	{
      ...
      error_code = heap_bestspace_fix_page (thread_p, hfid, &pages[i], &page_watcher);
      if (error_code != NO_ERROR)
	{
	  goto error_exit;
	}
      ...
      if (heap_copy_chain (thread_p, page_ptr, &chain) != NO_ERROR)
	{
      ...
      addr.pgptr = page_ptr;
      addr.offset = HEAP_HEADER_AND_CHAIN_SLOTID;
      log_append_redo_data (thread_p, RVHF_CHAIN, &addr, sizeof (chain), &chain);
      if (spage_update (thread_p, page_ptr, HEAP_HEADER_AND_CHAIN_SLOTID, &recdes) != SP_SUCCESS)
	{
	  error_code = ER_FAILED;
	  goto error_exit;
	}
			       ...
			       const cubstorage::bestspace_entry * update_entries, int num_update_entries)
{
  ...
  assert (update_entries && num_update_entries > 0);

  addr.vfid = &hfid->vfid;

  remaining_entries = num_update_entries;
  for (i = 0; i < (int) num_pages; i++)
      ...
      if (recdes.length <= 0 || recdes.length % sizeof (cubstorage::bestspace_entry) != 0)
	{
      ...
      num_entries_to_copy = MIN (remaining_entries, num_entries_in_page);
      if (num_entries_to_copy > 0)
	{
	  memcpy (entries, update_entries + entry_index, num_entries_to_copy * sizeof (cubstorage::bestspace_entry));
	  remaining_entries -= num_entries_to_copy;
	  entry_index += num_entries_to_copy;
	}
      for (j = num_entries_to_copy; j < num_entries_in_page; j++)
	{
	  entries[j].set_null ();
	}

      recdes.area_size = num_entries_in_page * sizeof (cubstorage::bestspace_entry);
      recdes.length = recdes.area_size;
      recdes.type = REC_HOME;
      recdes.data = (char *) entries;

      addr.pgptr = page_ptr;
      addr.offset = HEAP_BESTSPACE_ENTRIES_SLOTID;
      log_append_redo_recdes (thread_p, RVHF_UPDATE_BESTSPACE_ENTRIES, &addr, &recdes);
      if (spage_update (thread_p, page_ptr, HEAP_BESTSPACE_ENTRIES_SLOTID, &recdes) != SP_SUCCESS)
	{
	  error_code = ER_FAILED;
	  goto error_exit;
	}
...
// *INDENT-ON*
{
  ...
  bestspace->to_entries (entries, candidates, num_candidates);
  ...
  if (num_candidates > 0)
    {
      std::memcpy (heap_hdr->bestspace.candidates, candidates, num_candidates * sizeof (cubstorage::bestspace_entry));
      heap_hdr->bestspace.num_candidates = num_candidates;
    }
  /* shards */
...
static int
heap_update_bestspace_registry_entry (const HFID * hfid, cubstorage::bestspace * bestspace, void *args)
{
  THREAD_ENTRY *thread_p;

  assert (hfid != NULL);
  assert (bestspace != NULL);
  assert (args != NULL);

  thread_p = (THREAD_ENTRY *) args;
  return heap_update_bestspace (thread_p, hfid, bestspace);
}
...
/*
 * heap_load_bestspace () - Load a bestspace from heap file
 *   return:
 */
static int
heap_load_bestspace (THREAD_ENTRY * thread_p, HFID * hfid, const HEAP_HDR_STATS * header,
		     cubstorage::bestspace_entry * entries, cubstorage::bestspace_entry * candidates)
{
  PGBUF_WATCHER page_watcher;
  ...
  assert (header);
  assert (entries);
  assert (candidates || !header->bestspace.num_candidates);
  ...
  num_entries = 0;
  for (i = 0; remaining_entries > 0 && i < (int) header->bestspace.num_pages; i++)
    {
  ...
  /* copy the last persistent candidate checkpoint into the in-memory bestspace. */
  if (header->bestspace.num_candidates > 0)
    {
      memcpy (candidates, header->bestspace.candidates,
	      header->bestspace.num_candidates * sizeof (cubstorage::bestspace_entry));
    }
...
/*
 * heap_build_bestspace () - Load and rebuild a bestspace from heap file
 *   return:
 */
// *INDENT-OFF*
...
heap_build_bestspace (THREAD_ENTRY * thread_p, OID * class_oid, HFID * hfid, PGBUF_WATCHER * header_watcher)
{
  ...
  // *INDENT-ON*
  std::size_t num_entries, num_candidates, num_shards;
  ...
  error = heap_load_bestspace (thread_p, hfid, header, entries, candidates);
  if (error != NO_ERROR)
    {
      free_and_init (entries);
      if (candidates)
  ...
  cubstorage::bestspaces.create (hfid, num_shards, entries,
				 ...
				 num_candidates, header->num_pages, header->num_recs, header->recs_sumlen,
...
heap_find_bestspace (THREAD_ENTRY * thread_p, OID * class_oid, HFID * hfid, PGBUF_WATCHER * header_watcher)
{
  ...
  PGBUF_INIT_WATCHER (&watcher, PGBUF_ORDERED_HEAP_HDR, hfid);
  error = pgbuf_ordered_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_WRITE, &watcher);
  if (error != NO_ERROR)
    {
      ASSERT_ERROR_AND_SET (error);
      return NULL;
  ...
  /* build from the heap page */
  bestspace = heap_build_bestspace (thread_p, class_oid, hfid, &watcher);
  if (!bestspace)
    {
      pgbuf_ordered_unfix (thread_p, &watcher);
      return NULL;
...
heap_create_internal (THREAD_ENTRY * thread_p, HFID * hfid, const OID * class_oid, const bool reuse_oid)
{
  ...
  cubstorage::bestspace_entry header_page;
  PGBUF_WATCHER header_page_watcher;
  bool header_page_watcher_attached = false;
  int header_freespace;
  ...
  pgbuf_attach_watcher (thread_p, addr_hdr.pgptr, PGBUF_LATCH_WRITE, hfid, &header_page_watcher);
  header_page_watcher_attached = true;
  ...
  header_freespace =
    ...
    - (int) DB_ALIGN (sizeof (HEAP_HDR_STATS), HEAP_MAX_ALIGN) - heap_Slotted_overhead;
  header_page.freespace = (std::uint16_t) MAX (header_freespace, 0);
  header_page.volid = hfid->vfid.volid;
  header_page.pageid = hfid->hpgid;

  error_code = heap_create_bestspace (thread_p, hfid, &heap_hdr, &header_page);
  if (error_code != NO_ERROR)
    {
      ASSERT_ERROR ();
      goto error;
    }
  addr_hdr.pgptr = header_page_watcher.pgptr;
  assert (addr_hdr.pgptr != NULL);
  ...
  if (sp_success != SP_SUCCESS || slotid != HEAP_HEADER_AND_CHAIN_SLOTID)
  ...
  else
    {
      ...
      pgbuf_ordered_unfix (thread_p, &header_page_watcher);
      header_page_watcher_attached = false;
      addr_hdr.pgptr = NULL;
    }
...
error:
  ...
  if (header_page_watcher_attached)
    {
      if (header_page_watcher.pgptr != NULL)
	{
	  pgbuf_ordered_unfix (thread_p, &header_page_watcher);
	}
      addr_hdr.pgptr = NULL;
    }
  else if (addr_hdr.pgptr != NULL)
    {
...
{
  cubstorage::bestspace_entry first_entry;
  cubstorage::bestspace_entry candidate_entry;
  VPID vpid;			/* Volume and page identifiers */
  ...
  HEAP_CHAIN *chain;		/* Chain to next and prev page */
  PGBUF_WATCHER hdr_watcher;
  bool hdr_watcher_attached = false;
  HFID hfid_for_watcher;
  RECDES recdes;
  VPID last_vpid;
  int freespace;
  int is_header_page;
  ...
  hfid_for_watcher = *hfid;

  VPID_SET_NULL (&last_vpid);
  first_entry.set_null ();
  addr.vfid = &hfid->vfid;
  ...
  while (!(VPID_ISNULL (&vpid)))
      ...
      if (pgptr == NULL)
	{
      ...
      if (!is_header_page && heap_page_is_bestspace (thread_p, pgptr))
	{
	  if (heap_hdr == NULL)
	    {
	      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
      ...
      if (is_header_page)
	{
	  ...
	  heap_bestspace_clear_candidates (heap_hdr->bestspace.candidates, &heap_hdr->bestspace.num_candidates,
					   cubstorage::bestspace::MAX_CANDIDATES_QUEUE_SIZE);
	  first_entry.freespace = freespace;
	  first_entry.volid = vpid.volid;
	  first_entry.pageid = vpid.pageid;
	}
      else
	{
	  chain = (HEAP_CHAIN *) recdes.data;
	  ...
	  candidate_entry.freespace = freespace;
	  candidate_entry.volid = vpid.volid;
	  candidate_entry.pageid = vpid.pageid;
	  heap_bestspace_add_candidate (heap_hdr->bestspace.candidates, &heap_hdr->bestspace.num_candidates,
					cubstorage::bestspace::MAX_CANDIDATES_QUEUE_SIZE, &candidate_entry);
	}
  ...
  /*
   * Reset the reused header and log the change (RVHF_STATS), which also covers
   * the class_oid rewritten into heap_hdr above -- RVHF_REUSE_PAGE deliberately
   * skips the header slot and relies on this redo record.
   */
  VFID_SET_NULL (&heap_hdr->ovf_vfid);
  ...
  assert (heap_hdr->bestspace.num_pages > 0);
  pgbuf_attach_watcher (thread_p, hdr_pgptr, PGBUF_LATCH_WRITE, &hfid_for_watcher, &hdr_watcher);
  hdr_watcher_attached = true;

  if (heap_update_bestspace_chain
      (thread_p, &heap_hdr->class_oid, hfid, heap_hdr->bestspace.pages, heap_hdr->bestspace.num_pages) != NO_ERROR)
    {
      goto error;
    }
  ...
  if (hdr_watcher.page_was_unfixed)
    {
      hdr_pgptr = hdr_watcher.pgptr;
      heap_hdr = heap_get_header_stats_ptr (thread_p, hdr_pgptr);
      if (heap_hdr == NULL)
	{
	  goto error;
	}
  ...
  addr.pgptr = hdr_watcher.pgptr;
  addr.offset = HEAP_HEADER_AND_CHAIN_SLOTID;
  log_append_redo_data (thread_p, RVHF_STATS, &addr, sizeof (*heap_hdr), heap_hdr);
  pgbuf_set_dirty (thread_p, hdr_watcher.pgptr, DONT_FREE);
  pgbuf_ordered_unfix (thread_p, &hdr_watcher);
  hdr_watcher_attached = false;
  hdr_pgptr = NULL;
...
error:
  ...
  if (hdr_watcher_attached)
    {
      if (hdr_watcher.pgptr != NULL)
	{
	  pgbuf_ordered_unfix (thread_p, &hdr_watcher);
	}
      hdr_pgptr = NULL;
    }
  else if (hdr_pgptr != NULL)
    {
...
xheap_reclaim_addresses (THREAD_ENTRY * thread_p, const HFID * hfid)
{
  ...
  std::size_t max_candidates, max_bestpages;
  std::size_t num_entries;
  std::size_t i;
  ...
  num_entries = 0;
  entries =
    (cubstorage::bestspace_entry *) malloc ((max_candidates + max_bestpages) * sizeof (cubstorage::bestspace_entry));
  if (!entries)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1,
	      (max_candidates + max_bestpages) * sizeof (cubstorage::bestspace_entry));
      goto exit_on_error;
    }
  ...
  while (!(VPID_ISNULL (&prv_vpid)))
      ...
      if (heap_vpid_prev (thread_p, hfid, curr_page_watcher.pgptr, &prv_vpid) != NO_ERROR)
	{
      ...
      if (heap_page_is_bestspace (thread_p, curr_page_watcher.pgptr))
	{
	  pgbuf_ordered_unfix (thread_p, &curr_page_watcher);
	  continue;
	}
      ...
      else
	{
	  ...
	  heap_hdr.num_pages += npages;
	  heap_hdr.num_recs += nrecords;
	  heap_hdr.recs_sumlen += rec_length;

	  freespace = spage_get_free_space_without_saving (thread_p, curr_page_watcher.pgptr);

	  candidate.freespace = freespace;
	  candidate.volid = vpid.volid;
	  candidate.pageid = vpid.pageid;
	  heap_bestspace_add_candidate (entries, &num_entries, max_candidates + max_bestpages, &candidate);
  ...
  /* keep the rebuilt candidates as the latest persistent checkpoint. */
  ...
  /* update in-memory bestspace if it is resident */
...
{ // scan_cache의 hfid-class_oid가 유효한지 확인
  if (*scan_cache != NULL)
    {
      if ((*scan_cache)->debug_initpattern != HEAP_DEBUG_SCANCACHE_INITPATTERN)
	{
	  ...
	  *scan_cache = NULL;
      ...
      else if (!HFID_EQ (&(*scan_cache)->node.hfid, hfid) || OID_ISNULL (&(*scan_cache)->node.class_oid)) // 전달된 hfid가 scan_cache에 할당되어 있는지, scan_cache의 class_oid가 -1이 아닌지 검사
	{
	  int r;
...
{ // scan_cache 초기값 세팅
  ...
  /* scan_chae 초기값 세팅 */
  scan_cache->page_latch = PGBUF_LATCH_READ; // 초기값은 READ latch
...
{ // 전달된 class_oid로 scan_cahce 구성(hfid, ftype, lach 모드 X_LOCK으로 구성)
  ...
  scan_cache->page_latch = PGBUF_LATCH_WRITE; // scan_cache를 수정하기 위한 latch로 WRITE latch를 설정함
...
{ // scan_cache의 값들 정리(unfix 등등)하고 초기화해줌
  int ret = NO_ERROR;
...
{ // HEAP_GET_CONTEXT 준비작업을 해준다. context->oid fix, class_oid 획득, 레코드 타입에 따라 추가 작업
  // TODO: relocation/bigone 케이스는 무슨 작업 해주는지 확인해야 함
...
try_again:
  ...
  ret = heap_prepare_object_page (thread_p, context->oid_p, &context->home_page_watcher, context->latch_mode); // context->oid_p를 fix해줌
  if (ret != NO_ERROR)
    {
  ...
  /* Get slot. */
  slot_p = spage_get_slot (context->home_page_watcher.pgptr, context->oid_p->slotid); // oid의 slot에 해당하는 포인터 반환
  if (slot_p == NULL)
    {
      /* Slot doesn't exist. */
      if (!is_heap_scan)
  ...
  switch (slot_p->record_type)
    ...
    case REC_HOME: // 이 함수는 prepare만 해주는 함수이기 때문에 record 타입이 HOME이면 추가 작업 없이 success 반환
...
{ // context의 oid에 해당하는 mvcc_header 획득
  // TODO: 레코드 타입에 따라 다르게 동작하기 때문에 RELOCATION 타입 동작 확인해야 함
  RECDES peek_recdes;
  SCAN_CODE scan_code;
  ...
  switch (context->record_type)
    {
    case REC_HOME:
      scan_code = spage_get_record (thread_p, home_page, oid->slotid, &peek_recdes, PEEK); // oid에 해당하는 record를 peek_recdes에 획득
      if (scan_code != S_SUCCESS)
	{
      ...
      if (or_mvcc_get_header (&peek_recdes, mvcc_header) != NO_ERROR) // 위에서 획득한 peek_recdes로 mvcc_header 획득
	{
	  /* Unexpected. */
	  assert (false);
	  return S_ERROR;
	}
      return S_SUCCESS; // mvcc_header 얻었으니 이 함수에서의 역할은 끝났고 반환
    case REC_BIGONE:
...
{ // context의 oid에 해당하는 record를 획득함. ispeeking에 따라 alloc 여부가 결정되고, REALLOCATION은 무조건 alloc해주는듯
  // TODO: REALLOCATION/BIGONE 케이스 동작 방식 확인하기, alloc 과정 확인하기
  ...
  switch (context->record_type)
    ...
    case REC_HOME:
	  ...
	  && heap_scan_cache_allocate_recdes_data (thread_p, scan_cache_p, context->recdes_p, // ispeeking이 COPY면 record를 위한 alloc해주는 듯
						   DB_PAGESIZE * 2) != NO_ERROR)              // TODO: alloc 과정 확인하기
	{
	  ASSERT_ERROR ();
	  return S_ERROR;
	}
      return spage_get_record (thread_p, context->home_page_watcher.pgptr, context->oid_p->slotid, context->recdes_p,
			       context->ispeeking); // slot에서 oid에 해당하는 record 획득
    default:
      break;
    }
  /* Shouldn't be here. */
  ...
  while (true)
      ...
      while (true)
	{
	  ...
	  if (scan_cache->page_watcher.pgptr == NULL) // scan_cache의 page watcher가 NULL이면
		...
		heap_scan_pb_latch_and_fetch (thread_p, &vpid, OLD_PAGE_PREVENT_DEALLOC, PGBUF_LATCH_READ, scan_cache, // READ latch로 scan_cache->page_watcher fix해줌
	  ...
	  if (heap_page_is_bestspace (thread_p, scan_cache->page_watcher.pgptr))
		...
		{
		  if (heap_vpid_prev (thread_p, hfid, scan_cache->page_watcher.pgptr, &vpid) != NO_ERROR)
		    {
		      if (old_page_watcher.pgptr != NULL)
			{
			  pgbuf_ordered_unfix (thread_p, &old_page_watcher);
	      ...
	      else if (heap_vpid_next (thread_p, hfid, scan_cache->page_watcher.pgptr, &vpid) != NO_ERROR)
		{
		  if (old_page_watcher.pgptr != NULL)
		    {
		      pgbuf_ordered_unfix (thread_p, &old_page_watcher);
	  ...
	  if (get_rec_info) // 파라미터 cache_recordinfo로 전달되는 DB_VALUE 배열이 NULL이 아닌 경우
	      ...
	      while (true)
		{
		  if (reversed_direction) // 마지막부터 탐색하는 경우
		    ...
		    {
		      scan = spage_next_record (local_pgptr, &oid.slotid, &forward_recdes, PEEK); // 현재 oid의 다음 레크드를 forward_recdes에 담아옴
		    }
		  if (scan != S_SUCCESS)
		    {
		      /* stop */
	  ...
	  if (scan != S_SUCCESS) // scan에 실패했다면
	      ...
	      if (scan == S_END) // 다음 record가 없는 상황이라면
		{
		  ...
		  if (reversed_direction)
		    ...
		    {
		      (void) heap_vpid_next (thread_p, hfid, scan_cache->page_watcher.pgptr, &vpid); // vpid에 다음 페이지 지정
		    }
		  pgbuf_replace_watcher (thread_p, &scan_cache->page_watcher, &old_page_watcher);
		  oid.volid = vpid.volid;
		  oid.pageid = vpid.pageid;
      ...
      else // 파라미터 cache_recordinfo로 전달되는 DB_VALUE 배열이 NULL인 경우
	{
	  ...
	  scan =
	    heap_scan_get_visible_version (thread_p, &oid, class_oid, recdes, &forward_recdes, scan_cache, ispeeking,
					   NULL_CHN, is_cached_scan); // mvcc 기준으로 visible한 record를 recdes에 넣어옴(forward_recdes를 recdes에 그대로 복사하는 경우도 있음)
...
/*
 * heap_update_statistics () - Update heap object/page statistics from heap pages
 *   return: NO_ERROR if successful, error code otherwise
 *   hfid(in): Object heap file identifier
			...
			PGBUF_WATCHER * header_watcher)
{
  ...
  uint64_t num_recs, recs_sumlen;
  int npages, nrecords, reclength;
  int num_pages;
  int error_code = NO_ERROR;
  ...
  assert (hfid);
  assert (heap_hdr);
  assert (header_watcher && header_watcher->pgptr);
  ...
  spage_collect_statistics (header_watcher->pgptr, &npages, &nrecords, &reclength);
  num_pages = npages;
  num_recs = nrecords;
  ...
  while (!VPID_ISNULL (&vpid))
    {
      error_code = pgbuf_ordered_fix (thread_p, &vpid, OLD_PAGE_PREVENT_DEALLOC, PGBUF_LATCH_READ, &pg_watcher);
      if (error_code != NO_ERROR)
	{
	  ASSERT_ERROR_AND_SET (error_code);
	  goto exit_on_error;
      ...
      if (old_pg_watcher.pgptr != NULL)
	{
      ...
      error_code = heap_vpid_next (thread_p, hfid, pg_watcher.pgptr, &vpid);
      if (error_code != NO_ERROR)
	{
	  ASSERT_ERROR_AND_SET (error_code);
	  goto exit_on_error;
	...
	{
	  spage_collect_statistics (pg_watcher.pgptr, &npages, &nrecords, &reclength);
	  num_pages += npages;
	  num_recs += nrecords;
	  recs_sumlen += reclength;
...
heap_get_num_objects (THREAD_ENTRY * thread_p, const HFID * hfid, int *npages, int *nobjs, int *avg_length)
{
  ...
  /*
   * Get the heap header in exclusive mode and synchronize heap statistics.
   * The number of records/objects is updated by scanning all heap pages.
   */
  ...
  *nobjs = heap_hdr->num_recs;
  if (*nobjs > 0)
    {
      *avg_length = (int) ((heap_hdr->recs_sumlen / (float) *nobjs) + 0.9);
    }
  else
    {
      *avg_length = 0;
...
heap_estimate (THREAD_ENTRY * thread_p, const HFID * hfid, int *npages, int *nobjs, int *avg_length)
{
  ...
  RECDES hdr_recdes;		/* Record descriptor to point to space statistics */
  HEAP_HDR_STATS *heap_hdr;	/* Heap header */
  std::uint64_t recs_num, recs_sumlen;
  int num_pages;
  ...
  if (bestspace)
      ...
      if (spage_get_record (thread_p, hdr_pgptr, HEAP_HEADER_AND_CHAIN_SLOTID, &hdr_recdes, PEEK) != S_SUCCESS)
	{
      ...
      *npages = heap_hdr->num_pages;
      *nobjs = heap_hdr->num_recs;
      recs_sumlen = heap_hdr->recs_sumlen;
  ...
  if (*nobjs > 0)
    {
      *avg_length = (int) ((recs_sumlen / (float) *nobjs) + 0.9);
    }
  else
    {
      *avg_length = 0;
...
{ // attr_info의 초기값 세팅
  ...
  if (requested_num_attrs == 0) // select 경우 requested_num_attrs가 0이었음
    {
      /* initialize the attrinfo cache and return, there is nothing else to do */
      (void) memset (attr_info, '\0', sizeof (HEAP_CACHE_ATTRINFO)); // attr_info의 공간만 \0으로 초기화

      /* now set the num_values to -1 which indicates that this is an empty HEAP_CACHE_ATTRINFO and shouldn't be
       * operated on. */
      attr_info->num_values = -1; // 비어있는 상태임을 표시
      return NO_ERROR; // 아무것도 안 하고 반환
    }

  if (requested_num_attrs < 0) // insert 시 requested_num_attrs < 0이었음
    {
  ...
  else if (requested_num_attrs > // request 개수가 전체 컬럼 개수보다 많으면
  ...
  if (requested_num_attrs > 0)
    {
      attr_info->values = // attr_info->values 구조체에 attr 개수만큼 alloc해서 values 필드값 할당
  ...
  attr_info->num_values = requested_num_attrs; // 필요한 만큼 num_values 할당
  ...
  for (i = 0; i < attr_info->num_values; i++) // values 순회하면서 value 초기화
	...
	{
	  value->attrid = *attrids++; // 파라미터로 전달된 attrids 배열에 각 컬럼에 맞는 attrid가 들어 있어서 그걸 할당함
	}
  ...
  if (heap_attrinfo_recache_attrepr (attr_info, true) != NO_ERROR) // 위에서 만들어놓은 attr_info->value를 lastrepr 기준으로 모두 초기화해줌(attrid, type만 설정해준다고 보면 됨)
    {
      goto exit_on_error;
    }
...
{ // attr_info의 value를 순회하면서 attr_info->last/read를 기준으로 초기화(islast_reset이 true면 last 기준)
  // value는 dbvalue를 제외하고, id, type을 설정해준다고 보면 됨
  HEAP_ATTRVALUE *value;	/* Disk value Attr info for a particular attr */
  ...
  for (num_found_attrs = 0, curr_attr = 0; curr_attr < attr_info->num_values; curr_attr++) // attr_info의 모든 attr를 순회하면서 instance/shared/class 에 맞게 초기화해주는듯
      ...
      else
	{
      ...
      value = &attr_info->values[curr_attr]; // value를 0부터 끝까지 순회함
      ...
      if (value->attrid == -1)
	{
      ...
      else if (IS_DEDUPLICATE_KEY_ATTR_ID (value->attrid)) // deduplicate key를 위한 attrid라면. TODO
	{                                                  // deduplicate가 뭔지 모르겠음
	...
	{ // search_attrepr에서 instance attr 개수만큼 순회하면서 value->attrid와 동일한 컬럼을 찾으면 value값 초기화해주는듯
	   ...
	   */
	  if (value->attrid == search_attrepr->id) // 바깥 루프에서 순회하고 있는 value와 같은 value를 찾았다면 아래에서 초기화 진행
	    {
	      /*
	       * Found it.
...
{ // attrinfo 구조체 초기화
   ...
   */
  ret = heap_attrinfo_clear_dbvalues (attr_info); // attr_info의 value들 초기화
  ret = heap_attrinfo_recache (thread_p, NULL_REPRID, attr_info); // attr_info->read_classrepr NULL로 초기화
...
{ // attr_info의 values를 순회하면서 state가 HEAP_UNINIT_ATTRVALUE가 아닌 value들을 초기화해주는듯
  // TODO: value->state가 HEAP_UNINIT_ATTRVALUE인 케이스 확인
  HEAP_ATTRVALUE *value;	/* Disk value Attr info for a particular attr */
	  ...
	  if (value->state != HEAP_UNINIT_ATTRVALUE)
	    {
	       ...
	       * of the object
	       */
	      attrepr = ((value->read_attrepr != NULL) ? value->read_attrepr : value->last_attrepr); // value->read_attrepr이 있으면 readrepr로 
	      if (attrepr != NULL)
		{
		  if (pr_clear_value (&value->dbvalue) != NO_ERROR) // value->dbvalue 초기화
		    {
		      ret = ER_FAILED;
		    }
		  value->state = HEAP_UNINIT_ATTRVALUE; // uninit state 설정
		}
	    }
	}
    }
...
heap_attrvalue_point_fixed (RECDES * recdes, HEAP_CACHE_ATTRINFO * attr_info, OR_ATTRIBUTE * attrepr, RECDES * raw)
{
  ...
  raw->data = ((char *) recdes->data // fixed data 시작 지점 할당
	       + OR_FIXED_ATTRIBUTES_OFFSET_BY_OBJ (recdes->data,
						    attr_info->read_classrepr->n_variable) + attrepr->location);
  raw->length = tp_domain_disk_size (attrepr->domain); // disk 내에서 data가 저장된 사이즈
}
...
heap_attrvalue_point_variable (RECDES * recdes, HEAP_CACHE_ATTRINFO * attr_info, OR_ATTRIBUTE * attrepr, RECDES * raw)
{
  ...
  raw->data = ((char *) recdes->data + OR_VAR_OFFSET (recdes->data, attrepr->location)); // 그냥 data에서 variable offset만큼 떨어진 data 반환
...
heap_attrvalue_transform_to_dbvalue (HEAP_ATTRVALUE * value, OR_ATTRIBUTE * attrepr, RECDES * raw)
{
  ...
  if (raw->data == NULL) // data가 비어 있는 경우. TODO
    {
  ...
  else
    {
      ...
      pr_type = pr_type_from_id (attrepr->type); // data 타입 찾아옴
      if (pr_type)
	{
	  rv = pr_type->data_readval (&buf, &value->dbvalue, attrepr->domain, raw->length, false, NULL, 0); // disk에서 data 읽어서 dbvalue에 담아서 반환
	}
      value->state = HEAP_READ_ATTRVALUE;
      if (rv != NO_ERROR)
	{
...
heap_attrinfo_read_dbvalues (THREAD_ENTRY * thread_p, const OID * inst_oid, RECDES * recdes,
			     HEAP_CACHE_ATTRINFO * attr_info)
{ // recdes를 disk에서 읽어와서 attr_info->value에 담음
  int i;
  ...
  if (inst_oid != NULL && recdes != NULL && recdes->data != NULL)
      ...
      if (unlikely (attr_info->read_classrepr == NULL || attr_info->read_classrepr->id != reprid))
	{
	  ...
	  ret = heap_attrinfo_recache (thread_p, reprid, attr_info); // reprid에 맞는 repr로 attr_info 세팅
	  if (ret != NO_ERROR)
	    {
	      goto exit_on_error;
	    }
  ...
  for (i = 0; i < attr_info->num_values; i++) // attr_info + value 모두 세팅 끝났으니 read 시작
    {
      value = &attr_info->values[i];
      ret = heap_attrvalue_read (recdes, value, attr_info); // select * 했을 때는 last에 맞춰서 attr_info를 세팅해줬음
      if (ret != NO_ERROR)                                  // TODO: alter를 수행해서 read 버전을 필요로 한다면 어떤 동작을 하는지 확인
	{
	  goto exit_on_error;
	}
    }
...
{ // attrid에 해당하는 value 반환
  ...
  value = heap_attrvalue_locate (attrid, attr_info); // attr_info 순회해서 attrid에 해당하는 value 반환
...
{ // 전달된 attrid, dbvalue로 attr_info->value를 설정해줌. 값 세팅은 안 함
  // NIT: value를 setting하는 함수인데, 이름이 attrinfo로 되어 있어서 헷갈리는 것 같음. 누구냐!
  HEAP_ATTRVALUE *value;	/* Disk value Attr info for a particular attr */
...
heap_attrinfo_get_record_header_size (HEAP_CACHE_ATTRINFO * attr_info, int payload_size, bool is_mvcc_class,
				      size_t * offset_size_ptr)
{ // 데이터의 header 총 사이즈 계산
  int header_size;

  *offset_size_ptr = OR_BYTE_SIZE;

  header_size = is_mvcc_class ? OR_MVCC_INSERT_HEADER_SIZE : OR_NON_MVCC_HEADER_SIZE; // MVCC 사이즈 계산
  header_size += OR_VAR_TABLE_SIZE_INTERNAL (attr_info->last_classrepr->n_variable, *offset_size_ptr); // variable 데이터 테이블(? offset 테이블을 뜻하는 듯) 사이즈 계산
  header_size += OR_BOUND_BIT_BYTES (attr_info->last_classrepr->n_attributes - attr_info->last_classrepr->n_variable); // fixed 데이터 테이블(각 fixed 데이터가 NULL인지 여부를 나타내는 bit가 얼마나 필요한지 측정하는 것) 계산

  if (*offset_size_ptr == OR_BYTE_SIZE && header_size + payload_size > OR_MAX_BYTE) // variable 데이터의 크기와 표현해야 하는 offset 크기에 따라 바이트 수 계산하여 header 사이즈 조정
    {
...
{ // 전달된 buf에 header에 들어갈 정보(repr_id, chn, mvcc_ins_id)를 차례로 할당해줌
  ...
  if ((attr_info->last_classrepr->n_attributes - attr_info->last_classrepr->n_variable) != 0) // fix 데이터가 존재하면
    {
      repid_bits |= OR_BOUND_BIT_FLAG; // fix 비트 플래그 설정
    }
  ...
  /* refetch the object.                                            */
  attr_info->inst_chn++; // chn은 무조건 0으로 설정(왜?)
  ...
  if (is_mvcc_class)
      ...
      if (!is_update)
	{
	  ...
	  if ((buf->ptr + OR_MVCC_INSERT_HEADER_SIZE) > buf->endptr)
	    ...
	    {
	      or_put_int (buf, repid_bits); // 위에서 설정한 repid_bits를 buf(data) 포인터에 할당
	      or_put_int (buf, 0);	/* CHN */ // 위에서 설정한 repid_bits 다음으로 chn(0) 할당
	      or_put_bigint (buf, 0);	/* MVCC insert id */ // 위에서 설정한 chn 다음으로 buf에 MVCC_INS_ID 할당
	    }
	}
      else
	{
...

{
  ...
  buf->ptr = (buf->buffer
	      + OR_FIXED_ATTRIBUTES_OFFSET_BY_OBJ (buf->buffer, attr_info->last_classrepr->n_variable) // fixed 데이터 시작 지점 buf->ptr에 할당
	      + value->last_attrepr->location);
                // value->last_attrepr->location이 뭔지는 모르겠음
  if (value->do_increment && (incremented_attrids->find (index) == incremented_attrids->end ())) // auto_increment에 따라 값 설정하는 듯. TODO
    {
  ...
  if (dbvalue == NULL || db_value_is_null (dbvalue) == true) // dbvalue가 NULL이면
    {
      /*
	...
	{
	  or_pad (buf, tp_domain_disk_size (value->last_attrepr->domain)); // 값이 들어가야 할 자리에 전부 0으로 표기
	}
    }
  else
    {
       ...
       * Write the value.
       */
      if (buf->ptr + pr_type->get_disk_size_of_value (dbvalue) > buf->endptr) // 데이터 타입이 들어갈 자리가 충분한지 체크
	{
	  return S_DOESNT_FIT;
	}
      else // 충분하다면
	{
	  OR_ENABLE_BOUND_BIT (bitmap_bound, value->last_attrepr->position); // fixed_bitmap(bound_bits)에 value가 NULL이 아님을 1로 표시
                                           // value->last_attrepr->position가 뭔지 모르겠음
	  pr_type->data_writeval (buf, dbvalue); // 자세한 로직은 모르겠지만, disk에 write하는 포맷으로 buf에 dbvalue를 기록
	}
    }
...
heap_attrinfo_transform_variable_to_disk (THREAD_ENTRY * thread_p, HEAP_CACHE_ATTRINFO * attr_info, OR_BUF * buf,
					  ...
					  int lob_create_flag)
{ // variable data disk 폼으로 write가능하게 buf에 값 할당해줌
  // 1. variable offset table에 기록 2. dbvalue 값 disk 폼으로 buf에 기록
  HEAP_ATTRVALUE *value;
  DB_VALUE *dbvalue;
  ATTR_ID attrid;
  const PR_TYPE *pr_type;
  ...
  if ((buf->ptr + offset_size) > buf->endptr)
    ...
    {
      or_put_offset_internal (buf, CAST_BUFLEN (*ptr_varvals - buf->buffer - header_size), offset_size); // 1. variable offset 테이블에 offset 값 기록
    }
  ...
  if (dbvalue != NULL && db_value_is_null (dbvalue) != true)
	...
	{
	  pr_type->data_writeval (buf, dbvalue); // 2. dbvalue 값 disk 폼으로 buf에 기록
	  *ptr_varvals = buf->ptr;
	}
    }
...

{
  ...
  bitmap_bound = OR_GET_BOUND_BITS (buf->buffer, attr_info->last_classrepr->n_variable, // bound_bit의 시작 주소 반환
				    attr_info->last_classrepr->fixed_length);           // 여기서 잠깐!! bound_bit는 fixed 데이터들의 NULL 여부를 표현하는 비트이다.
  ptr_varvals = (bitmap_bound
		 + OR_BOUND_BIT_BYTES (attr_info->last_classrepr->n_attributes // bound_bit의 크기를 측정하여 variable 데이터 시작 지점을 반환함
				       - attr_info->last_classrepr->n_variable)); // bound_bit는 4바이트 단위로만 표현되기 때문에, 크기를 계산하는 것이다.

  for (i = 0; i < attr_info->num_values; i++)
    {
      if (attr_info->values[i].last_attrepr->is_fixed != 0) // fixed 데이터라면
	{
	  status =
	    heap_attrinfo_transform_fixed_to_disk (thread_p, attr_info, buf, i, incremented_attrids, bitmap_bound); // fixed 데이터 disk form으로 변환해서 buf에 저장
	}
      else // variable 데이터라면
	{
	  status =
	    heap_attrinfo_transform_variable_to_disk (thread_p, attr_info, buf, &ptr_varvals, i, offset_size, // variable 데이터 disk form으로 변환해서 buf에 저장
  ...
  if (attr_info->last_classrepr->n_variable > 0) // variable 데이터가 존재한다면
    {
      ...
      buf->ptr = ((char *) (OR_VAR_ELEMENT_PTR (buf->buffer, attr_info->last_classrepr->n_variable))); // varialbe-offset table 마지막 위치 반환. 이 위치가 variable-offset table 삽입 가능한 위치임
      if ((buf->ptr + offset_size) > buf->endptr)
	{
	  return S_DOESNT_FIT;
	}
	...
	{
	  or_put_offset_internal (buf, CAST_BUFLEN (ptr_varvals - buf->buffer - header_size), offset_size); // variable-offset table 마지막 원소에 variable data의 마지막 위치를 기록
	}                                                                                                   // variable data의 마지막 위치를 기록함으로써 마지막 variable 데이터 길이를 알 수 있음
  ...
  *record_size = ptr_varvals - buf->buffer; // 최종 레코드 크기 반환
...
{ // attr_info의 value를 disk form에 맞게 세팅해주는듯??
  ...
  if (heap_attrinfo_set_uninitialized (thread_p, &attr_info->inst_oid, old_recdes, attr_info) != NO_ERROR) // recdes가 NULL이면 attr_info->readrepr을 lastrepr로 바꿔주고,
    {                                                                                                      // recdes가 NULL이 아니면 attr_info->chn을 recdes에 맞게 설정해줌
      return S_ERROR;
    }
  ...
  /* determine the size */
  expected_size = heap_attrinfo_determine_disksize (attr_info, is_mvcc_class, &offset_size); // header+data 총 사이즈 계산
  ...
  if (is_mvcc_class)
      ...
      if (is_update)
	{
      ...
      expected_size += OR_MVCC_MAX_HEADER_SIZE - OR_MVCC_INSERT_HEADER_SIZE; // 왜 MVCC 최대 크기에서 MVCC_INSERT 사이즈를 뺀 크기를 expected_size에 더할까?
    }                                                                        // 추측: heap_attrinfo_determine_disksize()에서 이미 MVCC_INSERT_SIZE를 더했으니 이후에 더해질 가능성이 있는(DELID+PREV_LSA) flag들을 위해 공간을 확보해두는 것

  record_size = 0;
  do
    {
      /* build record buffer */
      new_recdes->resize_buffer (expected_size); // 자세히는 모르겠지만 recdes가 expected_size보다 공간이 부족하면 확장
      or_init (&buf, new_recdes->get_data_for_modify (), (int) expected_size);

      /* build header */
      status =
	heap_attrinfo_transform_header_to_disk (thread_p, attr_info, &buf, offset_size, is_mvcc_class, is_update); // header에 담길 내용 disk form으로 변경해서 buf에 할당(flag bit 설정해줘야 하는 부분들 등등)
      if (status == S_DOESNT_FIT)
	{
      ...
      /* build columns */
      status =
	heap_attrinfo_transform_columns_to_disk (thread_p, attr_info, &buf, &incremented_attrids, offset_size, // 각 column 데이터를 disk form으로 바꿔서 buf에 할당
  ...
  if (status == S_SUCCESS)
    {
      ...
      new_recdes->set_record_length (record_size); // record 사이즈 저장
    }
				...
				bool is_check_foreign)
{
  ...
  num_btids = &idx_info->num_btids; // 위에서 0으로 초기화했는데 다시 0을 할당하는 이 라인이 필요할까??

  set_attrids = guess_attrids;
  attr_info->num_values = -1;	/* initialize attr_info */

  classrepr = heap_classrepr_get (thread_p, class_oid, class_recdes, NULL_REPRID, &classrepr_cacheindex); // NULL_REPRID을 전달하면서 lastclassrepr 얻어옴
  if (classrepr == NULL)
    {
      return ER_FAILED;
    }

  if (classrepr->n_attributes > HEAP_GUESS_NUM_INDEXED_ATTRS) // attr_id 배열을 위해 최소 공간 100개를 마련하는데, 그보다 더 attr 개수가 많으면 malloc
    {
      set_attrids = (ATTR_ID *) malloc (classrepr->n_attributes * sizeof (ATTR_ID));
      if (set_attrids == NULL)
	{
  ...
  *num_btids = classrepr->n_indexes;
  ...
  for (j = 0; j < *num_btids; j++) // index(=btree) 개수만큼 반복. TODO
    {
  ...
  if (!idx_info->has_multi_col && num_found_attrs == 0) // 아무것도 찾지 못했을 때. 아마도 index 컬럼이 없어서 컬럼을 찾지 못한 케이스 같음. TODO
    {
      /* initialize the attrinfo cache and return, there is nothing else to do */
      ...
      /* now set the num_values to -1 which indicates that this is an empty HEAP_CACHE_ATTRINFO and shouldn't be
       * operated on. */
      attr_info->num_values = -1; // attr_info의 num_values를 -1로 설정하면서 초기화했다는걸 표시

      /* free the class representation */
      heap_classrepr_free_and_init (classrepr, &classrepr_cacheindex); // free
    }
  else
    {				/* num_found_attrs > 0 */
      /* initialize attribute information */
...
heap_chkreloc_next (THREAD_ENTRY * thread_p, HEAP_CHKALL_RELOCOIDS * chk, PAGE_PTR pgptr)
{
  ...
  if (heap_page_is_bestspace (thread_p, pgptr))
    {
      return DISK_VALID;
    }
...
{ // delete를 위한 vaccum log를 남김
  ...
  if (LOG_IS_MVCC_HEAP_OPERATION (rcvindex))
    {
      vacuum_status = heap_page_get_vacuum_status (thread_p, p_addr->pgptr); // heap_hdr냐 chain이냐에 따라 다른 status 반환

      heap_page_update_chain_after_mvcc_op (thread_p, p_addr->pgptr, logtb_get_current_mvccid (thread_p)); // chain인 경우에는 어떤 값을 update해줌. TODO: heap_chain인 경우 체크
      if (heap_page_get_vacuum_status (thread_p, p_addr->pgptr) != vacuum_status) // vaccum_state 다시 확인. 왜 다시 확인하는지는 모르겠음
	{
  ...
  if (thread_p->no_logging)
    ...
    {
      log_append_undoredo_data (thread_p, rcvindex, p_addr, 0, redo_data_size, NULL, redo_data_p); // undo를 위해 logging 남겨놓음
    }
}
		...
		DB_VALUE ** cache_pageinfo)
{
  ...
  while (true)
      ...
      if (!heap_page_is_bestspace (thread_p, pg_watcher.pgptr))
	{
      ...
      if (heap_vpid_next (thread_p, hfid, pg_watcher.pgptr, next_vpid) != NO_ERROR)
	{
	  pgbuf_ordered_unfix (thread_p, &pg_watcher);
	  return S_ERROR;
	}
      ...
      if (VPID_ISNULL (next_vpid))
	{
      ...
      /* keep the bestspace page fixed while following the link so vacuum cannot relink past it */
      pgbuf_replace_watcher (thread_p, &pg_watcher, &old_pg_watcher);
    }
		...
		DB_VALUE ** cache_pageinfo)
{
  ...
  while (true)
      ...
      if (!heap_page_is_bestspace (thread_p, pg_watcher.pgptr))
	{
      ...
      if (heap_vpid_prev (thread_p, hfid, pg_watcher.pgptr, prev_vpid) != NO_ERROR)
	{
	  pgbuf_ordered_unfix (thread_p, &pg_watcher);
	  return S_ERROR;
	}
...
heap_get_class_oid_from_page (THREAD_ENTRY * thread_p, PAGE_PTR page_p, OID * class_oid)
{
  ...
  if (spage_get_record (thread_p, page_p, HEAP_HEADER_AND_CHAIN_SLOTID, &chain_recdes, PEEK) != S_SUCCESS)
    { // 전달된 page의 첫 slot(SLOT_HEADER)을 chain_recdes에 획득
      assert (0);
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
      return ER_FAILED;
    }

  chain = (HEAP_CHAIN *) chain_recdes.data; // 첫 slot의 데이터 포인터를 chain으로 할당
  COPY_OID (class_oid, &(chain->class_oid)); // chain에 담긴 class_oid를 반환할 class_oid에 할당
  ...
  if (OID_ISNULL (class_oid))
    {
      ...
      COPY_OID (class_oid, oid_Root_class_oid); // class_oid가 NULL이면 root_class_oid로 대체
    }
  return NO_ERROR;
}
...
{ // root_hfid를 찾고, scancache 생성
  HFID root_hfid;

  (void) boot_find_root_heap (&root_hfid); // root_hfid 찾기
  (void) heap_scancache_quick_start_internal (scan_cache, &root_hfid); // root_hfid를 기준으로 scan_cache 생성
  scan_cache->page_latch = PGBUF_LATCH_READ;
...
{ // file_type 찾아서 반환
  FILE_TYPE file_type;
  if (context->scan_cache_p != NULL) // context->scan_cache_p가 있으면
    {
      assert (HFID_EQ (&context->hfid, &context->scan_cache_p->node.hfid));
      assert (context->scan_cache_p->file_type != FILE_UNKNOWN_TYPE);

      return context->scan_cache_p->file_type; // 그냥 그 안에 있는 ftype 반환
    }
  else
    {
      if (heap_get_class_info (thread_p, &context->class_oid, NULL, &file_type, NULL) != NO_ERROR) // class_oid로 ftype 얻어옴
	{
	  ASSERT_ERROR ();
 ...
 */
static int
heap_is_valid_oid (THREAD_ENTRY * thread_p, OID * oid_p) // 전달된 oid가 유효한 oid인지 확인
{
  DISK_ISVALID oid_valid = HEAP_ISVALID_OID (thread_p, oid_p); // oid가 hdr나 chain은 아닌지, disk에 예약되어 있는 oid를 가리키고 있는지 확인
...
heap_insert_adjust_recdes_header (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * insert_context, bool is_mvcc_class)
{
  ...
  repid_and_flag_bits = OR_GET_MVCC_REPID_AND_FLAG (insert_context->recdes_p->data); // 아마 data header에 MVCC 정보가 있는데 이걸 가져오는 것 같음
  mvcc_flags = (repid_and_flag_bits >> OR_MVCC_FLAG_SHIFT_BITS) & OR_MVCC_FLAG_MASK; // mvcc_flag 연산해서 mvcc_flags에 할당
  ...
  if (use_optimization) // record에 들어갈 mvcc 값을 조정해주는 것 같음
    {
      /*
      ...
      mvcc_id = logtb_get_current_mvccid (thread_p); // 모든 트랜잭션이 mvccid를 공유하는 것 같은데, 그 id를 하나 부여받음. 부여받은 id는 고유함
      ...
      if (!(mvcc_flags & OR_MVCC_FLAG_VALID_INSID))
	{
      ...
      /* Sets the MVCC INSID */
      OR_PUT_BIGINT (new_ins_mvccid_pos_p, &mvcc_id); // mvcc_id에 뭘 하는 것 같은데 지금 보기에는 너무 복잡하고 어려움. TODO
...
heap_insert_handle_multipage_record (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context)
{
  ...
  if (!heap_is_big_length (context->recdes_p->length)) // OVERFLOW 대상이 아니라면
    {
      return NO_ERROR; // 아무 작업도 하지 않고 return
    }
				    ...
				    PGBUF_WATCHER * home_hint_p)
{
  ...
  if (home_hint_p == NULL) // home hint가 없으면
    {
      assert (!HFID_IS_NULL (&context->hfid));

      /* find and fix page for insert */
      if (heap_find_bestpage (thread_p, &context->class_oid, &context->hfid, context->recdes_p->length, true, // best page 찾아서 fix해줌
  ...
  context->res_oid.volid = pgbuf_get_volume_id (context->home_page_watcher_p->pgptr); // context->res_oid는 result_oid로 insert할 위치를 나타내는 것 같음
  ...
  for (slot_id = 0; slot_id <= slot_count; slot_id++)
      ...
      if (slot_id == SP_ERROR)
	{
      ...
      context->res_oid.slotid = slot_id; // context->res_oid는 result_oid로 insert할 위치를 나타내는 것 같고, 삽입 가능한 slot 위치를 찾았으니 slotid를 결정해준듯
      ...
      if (!mvcc_is_mvcc_disabled_class (&context->class_oid))
	{
      ...
      lk_result = lock_object (thread_p, &context->res_oid, &context->class_oid, lock, LK_COND_LOCK); // context->res_oid에 lock 모드에 맞는 lock을 걸어서 반환
      if (lk_result == LK_GRANTED) // 문제 없이 lock을 획득했다면
	{
	  /* successfully locked! */
	  return NO_ERROR; // NO_ERROR 반환하면서 함수 종료
	}
...
{ // context의 oid에 해당하는 vpid를 fix해주고 X_LOCK 획득. 이미 fix되어 있는 상태라면 패스
  VPID vpid;
  ...
  vpid.pageid = context->oid.pageid; // context에 있는 oid를 vpid로 변환
  vpid.volid = context->oid.volid;

  /* first try to retrieve cached fixed page from scancache */
  if (context->scan_cache_p != NULL && context->scan_cache_p->page_watcher.pgptr != NULL // page가 fix되어 있고, fix를 유지해야 한다면
  ...
  if (context->home_page_watcher_p->pgptr == NULL)
    {
      (void) heap_scan_pb_latch_and_fetch (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_WRITE, context->scan_cache_p, // oid에 해당하는 vpid를 fix해주고, WRITE latch로 lock해줌
					   context->home_page_watcher_p);
      if (context->home_page_watcher_p->pgptr == NULL) // fix가 안 되어 있으면 에러
	{
	  int rc;
...
#if !defined(NDEBUG)
  if (context->scan_cache_p != NULL) // fix된 page에서 class_oid를 획득하고 정상인지 확인. 딱히 영향 있는 동작은 없음
    {
      OID heap_class_oid;

      assert (heap_get_class_oid_from_page (thread_p, context->home_page_watcher_p->pgptr, &heap_class_oid) == // fix된 page의 hdr or chain에 접근해서 class_oid 획득
	      NO_ERROR);
      assert ((OID_EQ (&heap_class_oid, &context->scan_cache_p->node.class_oid)) // 위에서 얻은 class_oid가 정상인지 확인
...
heap_delete_home (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op)
{
  ...
  /* operation */
  if (is_mvcc_op) // mvcc가 활성화 되어 있다면
    {
      ...
      repid_and_flag_bits = OR_GET_MVCC_REPID_AND_FLAG (context->home_recdes.data); // home_recdes.data에서 4바이트 읽어서 int로 번환
      mvcc_flags = (repid_and_flag_bits >> OR_MVCC_FLAG_SHIFT_BITS) & OR_MVCC_FLAG_MASK; // repid_and_flag_bits에서 mvcc_flag만 떼어내서 mvcc_flags에 저장
      adjusted_size = context->home_recdes.length;

      /* Uses the optimization in most common cases, for now : if DELID not set and adjusted size is not big size. */
      use_optimization = true;
      if (!(mvcc_flags & OR_MVCC_FLAG_VALID_DELID)) // mvcc_flag에 DELID가 설정되어 있지 않으면
	{
	  adjusted_size += OR_MVCCID_SIZE; // 길이에 MVCCID_SIZE 만큼 추가. delete를 처음하게 되면 MVCC_DELID가 추가돼서 그런 것 같음
	  is_adjusted_size_big = heap_is_big_length (adjusted_size);
	  if (is_adjusted_size_big)
	    {
	      /* Rare case, do not optimize it now. */
	      use_optimization = false; // overflow 사이즈면 최적화는 하지 않는듯
	    }
	}
      else
	{
...
#if !defined(NDEBUG)
      if (is_adjusted_size_big) // overflow의 adjust가 필요하면
	{
	  /* not exactly necessary, but we'll be able to compare sizes */
	  adjusted_size = context->home_recdes.length - mvcc_header_size_lookup[mvcc_flags] + OR_MVCC_MAX_HEADER_SIZE; // adjusted_size 설정
	} // mvcc_header_size_lookup는 각 인덱스별로 mvcc_header 사이즈가 정해져 있음
#endif

      /* Build the new record. */
      HEAP_SET_RECORD (&built_recdes, IO_DEFAULT_PAGE_SIZE + OR_MVCC_MAX_HEADER_SIZE, 0, REC_UNKNOWN, // 로컬 built_recdes에 파라미터로 전달한 값들을 설정해주는데 왜 하는건지 모르겠음
		       PTR_ALIGN (data_buffer, MAX_ALIGNMENT));
      if (use_optimization) // DELID를 활성화 해줘야 한다면 레코드 전체를 수정하는게 아니라 flag_bit의 DELID 비트만 활성화하도록 최적화만 해줌
	{ // 그걸 위해 최적화 작업 수행
	  char *start_p;

	  delid_offset = OR_MVCC_DELETE_ID_OFFSET (mvcc_flags); // mvcc_flag에서 계산해서 DELID_OFFSET 위치를 찾아줌.

	  build_recdes_data = start_p = built_recdes.data;

	  /* Copy up to MVCC DELID first. */
	  memcpy (build_recdes_data, context->home_recdes.data, delid_offset); // build_recdes_data에 DELID_OFFSET 직전까지 복사
	  build_recdes_data += delid_offset; // build_recdes_data의 포인터를 DELID 시작 지점으로 이동. 이 지점부터 DELID 삽입 예정

	  /* Sets MVCC DELID flag, overwrite first four bytes. */
	  repid_and_flag_bits |= (OR_MVCC_FLAG_VALID_DELID << OR_MVCC_FLAG_SHIFT_BITS); // 전체(repid+mvcc) FLAG에서 or 연산으로 delete 비트 활성화해서 repid_and_flag_bits에 할당
	  OR_PUT_INT (start_p, repid_and_flag_bits); // 전체 built_recdes.data에서 repid_and_flag_bits 만큼만 수정된 값으로 변경

	  /* Sets the MVCC DELID. */
	  OR_PUT_BIGINT (build_recdes_data, &mvcc_id); // 위에서 delid 위치로 build_recdes_data의 포인터를 옮겼음. 거기에 mvcc_id 할당. 이 mvcc_id가 DELID가 됨
	  build_recdes_data += OR_MVCC_DELETE_ID_SIZE; // DELID 입력 끝났으니 포인터를 DELID 끝 지점으로 이동
	  ...
	  memcpy (build_recdes_data, context->home_recdes.data + delid_offset, // data에서 delid 직전까지를 제외하고, delid 이후 부터 삽입
      ...
      if (is_adjusted_size_big)
	{
      ...
      else if (!spage_is_updatable (thread_p, context->home_page_watcher_p->pgptr, context->oid.slotid, // delid를 넣었더니 사이즈가 모자라면
				    built_recdes.length))
	{
	  built_recdes.type = REC_NEWHOME; // newhome으로 타입 변경
	}
      else
	{
	  built_recdes.type = REC_HOME; // 사이즈 충분하면 home으로 타입 설정
	}

      HEAP_PERF_TRACK_EXECUTE (thread_p, context);

      /* check whether relocation is necessary */
      if (built_recdes.type == REC_BIGONE || built_recdes.type == REC_NEWHOME) // record 위치를 옮겨야 한다면 TODO
	{
	  /*
      ...
      else
	{
	  ...
	  heap_mvcc_log_delete (thread_p, &rec_address, RVHF_MVCC_DELETE_REC_HOME); // delete를 위한 mvcc 로깅

	  HEAP_PERF_TRACK_LOGGING (thread_p, context);

	  /* we'll update the home page with the built record, since it fits in home page */
	  home_page_updated_recdes = &built_recdes; // 위에서 delid 삽입해서 만든 recdes
      ...
      error_code =
	heap_update_physical (thread_p, context->home_page_watcher_p->pgptr, context->oid.slotid, // delete여도 mvcc가 사용되면 delid가 추가된 record로 update가 수행됨
...
heap_update_physical (THREAD_ENTRY * thread_p, PAGE_PTR page_p, short slot_id, RECDES * recdes_p)
{
  ...
  old_record_type = spage_get_record_type (page_p, slot_id); // update 전 rec->type 저장

  /* update home page and check operation result */
  scancode = spage_update (thread_p, page_p, slot_id, recdes_p); // update 수행
  ...
  if (old_record_type != recdes_p->type)
    {
      spage_update_record_type (thread_p, page_p, slot_id, recdes_p->type); // update 중 realloc으로 변하거나 기타 등등으로 레코드 타입이 변하면 타입 업데이트
    }

  /* mark as dirty */
  pgbuf_set_dirty (thread_p, page_p, DONT_FREE); // dirty 설정
...
heap_insert_logical (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context, PGBUF_WATCHER * home_hint_p) // 일반적인 insert 케이스에는 home_hint_p가 NULL임. 다른 케이스는 NULL이 아닐 수 있음(TODO)
{
  ...
  if (!OID_ISNULL (&context->class_oid) && !OID_IS_ROOTOID (&context->class_oid) // context->class_oid가 NULL이 아니고, rootoid가 아니면
    ...
    {
      if (heap_insert_adjust_recdes_header (thread_p, context, is_mvcc_class) != NO_ERROR) // insert를 위해 record header의 mvcc 값을 조정해주는 듯
	{
	  return ER_FAILED;
	}
    }
   ...
   */
  if (heap_insert_handle_multipage_record (thread_p, context) != NO_ERROR) // OVERFLOW 대상인지 확인하고, 대상이면 다른 작업을 해줌. TODO: OVERFLOW 대상인 경우 체크
    {
      rc = ER_FAILED;
      goto error;
    }
  ...
  if (context->is_bulk_op)
  ...
  else
    {
      ...
      if (lock_object (thread_p, &context->class_oid, oid_Root_class_oid, IX_LOCK, LK_UNCOND_LOCK) != LK_GRANTED) // 자세히는 모르겠지만, class_oid에만 lock을 거는 듯
	{
	  return ER_FAILED;
	}
    }
  ...
  if (heap_get_insert_location_with_lock (thread_p, context, home_hint_p) != NO_ERROR) // best_page를 찾거나 새로운 page를 만들고, 해당 page+slotid에 lock을 걸어줌
    {
      return ER_FAILED;
    }
...
{ // 함수가 시작될 때 oid에 이미 삭제할 slot_id까지 정해진 상태로 들어옴
  bool is_mvcc_op;
  int rc = NO_ERROR;
  PERF_UTIME_TRACKER time_track;
  ...
  if (heap_is_valid_oid (thread_p, &context->oid) != NO_ERROR) // context->oid가 유효한 oid인지 확인
    {
      return ER_FAILED;
    }

  /* check scancache */
  if (heap_scancache_check_with_hfid (thread_p, &context->hfid, &context->class_oid, &context->scan_cache_p) != // scan_cache의 hfid-class_oid가 유효한지 확인
      NO_ERROR)
    {
      return ER_FAILED;
    }
  ...
  /* check file type */
  context->file_type = heap_get_file_type (thread_p, context); // ftype 찾아서 반환
  // 일반적인 delete의 경우에는 context->scan_cache가 있어서 ftype 바로 반환함
  if (context->file_type != FILE_HEAP && context->file_type != FILE_HEAP_REUSE_SLOTS) // file_type이 heap이 아니면 에러
    {
   ...
   * Class deletion case
   */
  if (HFID_EQ (&context->hfid, &(heap_Classrepr->rootclass_hfid))) // delete하려는 record가 rootclass에 속한거라면(TODO)
    {
   ...
   */
  if (heap_get_record_location (thread_p, context) != NO_ERROR) // context의 oid에 해당하는 vpid fix하고 X_LOCK 획득
    {
      rc = ER_FAILED;
      goto error;
    }

  context->record_type = spage_get_record_type (context->home_page_watcher_p->pgptr, context->oid.slotid); // record_type 획득. REC_HOME/REC_NEWHOME...
  ...
  context->home_recdes.data = PTR_ALIGN (context->home_recdes_buffer, MAX_ALIGNMENT); // context의 home_recdes_buffer의 주소에 따라서 home_recdes.data 주소를 align 맞춤
  if (spage_get_record
      (thread_p, context->home_page_watcher_p->pgptr, context->oid.slotid, &context->home_recdes, COPY) != S_SUCCESS) // slotid의 record를 home_recdes에 얻어옴
    {
      rc = ER_FAILED;
      goto error;
    }
  ...
  switch (context->record_type)
    ...
    case REC_HOME:
    case REC_ASSIGN_ADDRESS:
      rc = heap_delete_home (thread_p, context, is_mvcc_op); // home 데이터 delete
      break;
...
{ // 전달된 page의 0번 slot이 chain인 경우 chain의 어떤 값을 바꿔줌
  HEAP_CHAIN *chain;
  RECDES chain_recdes;
  HEAP_PAGE_VACUUM_STATUS vacuum_status;
  ...
  if (chain_recdes.length != sizeof (HEAP_CHAIN)) // heap_hdr의 경우
    {
      /* Heap header page. Do nothing. */
      assert (chain_recdes.length == sizeof (HEAP_HDR_STATS));
      return; // 그냥 리턴
    }
  chain = (HEAP_CHAIN *) chain_recdes.data; // heap_chain의 경우 TODO
...
heap_page_get_vacuum_status (THREAD_ENTRY * thread_p, PAGE_PTR heap_page)
{
  ...
  /* Get heap chain. */
  if (spage_get_record (thread_p, heap_page, HEAP_HEADER_AND_CHAIN_SLOTID, &chain_recdes, PEEK) != S_SUCCESS) // heap_hdr, chain 얻어오기
    {
      assert_release (false);
      return HEAP_PAGE_VACUUM_UNKNOWN;
    }
  if (chain_recdes.length != sizeof (HEAP_CHAIN)) // chain이 아니라 heap_hdr라면
    {
      /* Heap header page. */
      assert (chain_recdes.length == sizeof (HEAP_HDR_STATS));
      return HEAP_PAGE_VACUUM_UNKNOWN; // 이런 status로 반환
    }
  chain = (HEAP_CHAIN *) chain_recdes.data; // chain이라면 TODO
			       ...
			       bool is_cached_scan)
{
   ...
   */
  if (peeked_recdes->type == REC_HOME && (ispeeking == PEEK || ispeeking == COPY)) // REC_HOME이고 PEEK 또는 COPY면
    {
      ...
      if (class_oid != NULL) // class_oid가 NULL이 아니면
	{
	  ...
	  else if (!scan_cache->mvcc_disabled_class) // scan_cache의 mvcc가 가능이면
	    {
...
{ // record의 mvcc->chn을 확인해서 최신 버전을 context->recdes에 반환함
  SCAN_CODE scan = S_SUCCESS;
  MVCC_REC_HEADER mvcc_header = MVCC_REC_HEADER_INITIALIZER;
  ...
  scan = heap_prepare_get_context (thread_p, context, false, LOG_WARNING_IF_DELETED); // 파라미터로 전달하는 context에 대한 작업을 해줌(oid fix, class_oid 획득)
  if (scan != S_SUCCESS)
    {
      goto exit;
    }
  ...
  scan = heap_get_mvcc_header (thread_p, context, &mvcc_header); // mvcc_header 획득
  if (scan != S_SUCCESS)
    {
      goto exit;
    }

  if (MVCC_IS_CHN_UPTODATE (&mvcc_header, context->old_chn)) // mvcc와 context->chn을 비교해서 버전이 같으면 true
    {
      /* Object version didn't change and CHN is up-to-date. Don't get record data and return
       * S_SUCCESS_CHN_UPTODATE instead. */
      scan = S_SUCCESS_CHN_UPTODATE;
  ...
  if (context->recdes_p != NULL) // recdes_p 공간은 있는데 아직 준비가 안 됐다면
    {
      scan = heap_get_record_data_when_all_ready (thread_p, context); // context->oid_p에 해당하는 record를 획득해옴
    }
...
{ // TODO: oid 정보를 fix해주는 것 같은데, 자세히 다시 봐야 함
  VPID object_vpid;
  int ret = NO_ERROR;
...
{ // fix 부분이라 잘 모르겠지만, context에서 사용한 home/fwd 페이지들을 원상복구해주는듯(unfix인듯)
  assert (context != NULL);
...
{ // 전달된 파라미터들을 context에 할당해줌
  context->oid_p = oid; //context에 oid 할당
  OID_SET_NULL (&context->forward_oid);
  context->class_oid_p = class_oid; // context에 class_oid 할당
  context->recdes_p = recdes; // context에 recdes 할당
  ...
  context->scan_cache = scan_cache; // context에 scancache 할당
  context->ispeeking = ispeeking; // context에 ispeeking 여부 할당
  context->old_chn = old_chn; // context에 old_chn 할당
  if (scan_cache != NULL && scan_cache->page_latch == PGBUF_LATCH_WRITE) // context에 LATCH 모드에 따라 값 할당
    {
...
#endif /* !NDEBUG */
  heap_init_get_context (thread_p, &context, class_oid, &root_oid, recdes_p, scan_cache, ispeeking, NULL_CHN); // context에 여러가지 값(파라미터) 할당
...
heap_alloc_new_pages (THREAD_ENTRY * thread_p, HFID * hfid, int npages, VPID * new_page_vpids, PGBUF_WATCHER * new_pg_watcher)
{
  ...
  heap_hdr->last_vpid = new_page_vpids[npages - 1];
  heap_hdr->num_pages += npages;
			    ...
			    const int new_num_pages)
{
  ...
  heap_hdr->last_vpid = new_last_vpid;
  heap_hdr->num_pages += new_num_pages;
